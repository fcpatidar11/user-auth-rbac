const mongoose = require("mongoose");
const { UserContact } = require("@database/app.database");
const ObjectId = mongoose.Types.ObjectId;

module.exports = {
    create: async (insertData) => {
        try {
            return await UserContact.create({
                ...insertData,
                ...(insertData && insertData.user && { user: new ObjectId(insertData.user) })
            });
        } catch (error) {
            throw error;
        }
    },
    update: async (filterData, updateData) => {
        try {
            return await UserContact.findOneAndUpdate({
                ...Object.fromEntries(
                    Object.entries(filterData).filter(([_, v]) => v != null && v !== '')
                ),
                ...(filterData.user && { user: new ObjectId(filterData.user) }),
                ...(filterData._id && { _id: new ObjectId(filterData._id) })
            }, {
                $set: updateData
            }, { new: true });
        } catch (error) {
            throw error;
        }
    },
    retrieve: async (filterData) => {
        try {
            return await UserContact.findOne({
                ...Object.fromEntries(
                    Object.entries(filterData).filter(([_, v]) => v != null && v !== '')
                ),
                ...(filterData.user && { user: new ObjectId(filterData.user) }),
                ...(filterData._id && { _id: new ObjectId(filterData._id) })
            });
        } catch (error) {
            throw error;
        }
    },
    deleteOne: async (filterData) => {
        try {
            return await UserContact.findOneAndDelete({
                ...Object.fromEntries(
                    Object.entries(filterData).filter(([_, v]) => v != null && v !== '')
                ),
                ...(filterData.user && { user: new ObjectId(filterData.user) }),
                ...(filterData._id && { _id: new ObjectId(filterData._id) })
            });
        } catch (error) {
            throw error;
        }
    },
    retrieves: async (filterData) => {
        try {
            const query = [
                {
                    $match: {
                        ...Object.fromEntries(
                            Object.entries(filterData).filter(([_, v]) => v != null && v !== '')
                        ),
                        ...(filterData.user && { user: new ObjectId(filterData.user) }),
                        ...(filterData._id && { _id: new ObjectId(filterData._id) })
                    }
                }
            ];
            return await UserContact.aggregate(query);
        } catch (error) {
            throw error;
        }
    }
}